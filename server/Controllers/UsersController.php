...
class UsersController
{
    ...
    public function signUp()
    {
        ...
        if ($signUpSucces) {
            return json_encode(['message' => 'User Registed succesfully', 'status' => 'success']);
        } else {
            return json_encode(['message' => 'UserName Already Exist', 'status' => 'failed']);
    ...
    public function logIn()
    {
        ...
        if ($user && password_verify($password, $user['password_hash'])) {
            return json_encode(['user' => $user, 'status' => 'success' , 'message'=>"success"]);
        } else {
            return json_encode(['message' => "UserName Or password Wrong", 'status' => 'failed']);
        }
    }
}
